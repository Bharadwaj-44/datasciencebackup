...
class CodeKernel(object):
    def __init__(self,
        ...
        # ✅ CRITICAL FIX: Set matplotlib backend BEFORE kernel starts!

        print("🔧 Configuring environment for headless EC2 operation...")

        os.environ['MPLBACKEND'] = 'Agg'

        os.environ['DSA_SESSION_CACHE_PATH'] = session_cache_path

        # Prevent any GUI attempts

        os.environ['DISPLAY'] = ''

        os.environ['QT_QPA_PLATFORM'] = 'offscreen'

        print("✅ Environment: MPLBACKEND=Agg, DISPLAY='' (headless mode)")
    ...
    def execute_code_(self, code):
        ...
        # Only inject custom show if code actually displays plots

        if 'plt.show' in code or 'plt.savefig' in code or '.show()' in code:

            print("🎨 Chart detected! Injecting custom save function...")
...
# Matplotlib should already use Agg from environment variables
if 'matplotlib.pyplot' not in dir():
    import matplotlib.pyplot as plt
 
# Custom show function to save plots
def custom_show(*args, **kwargs):
    '''Save plots to files instead of showing GUI'''
    fig = plt.gcf()
    if fig.get_axes():
...
"""
        ...
        # Execute the code (with or without injection)
